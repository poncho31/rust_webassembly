use web_sys::{window, console, HtmlInputElement, Event, Document, Element, Window, FormData};
...
pub fn form_init(form_id: &str, endpoint: &str, field_specs: Option<&[(&str, FieldType)]>) -> Result<(), JsValue> {
    ...
    let form    : Element  = document.get_element_by_id(form_id).ok_or_else(|| JsValue::from_str(&format!("Form id not found : #{}",form_id)))?;
